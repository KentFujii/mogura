RSpec.describe Mogura::Init do
  it "generate a sample dig" do
    ...
    allow(FileUtils).to receive(:mkdir_p).with(pathname.join('app/views/test').to_s)
    io = double('io')
    allow(File).to receive(:new).with(pathname.join('app/views/test/sample.json.jbuilder').to_s, 'w').and_return(io)
